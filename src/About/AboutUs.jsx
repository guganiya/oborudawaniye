...
import { Award, Globe, Users, Zap, Quote, Linkedin, Mail, ChevronLeft, ChevronRight } from 'lucide-react'
...
const CompanyStats = () => {
	const stats = [
		...
		{ label: 'Патентов', value: '150+' },
		{ label: 'Стран', value: '100+' },
...
const ValueCard = ({ icon, title, text, delay }) => (
	...
	>
		<div className='text-gray-300 group-hover:text-[#e21e26] transition-colors duration-500 mb-6 transform group-hover:scale-110'>
			{icon}
		</div>
	...
	useEffect(() => {
		const fetchTeams = async () => {
			...
			try {
				...
				const teamsData = data.results || data.items || data
				setTeams(teamsData)
				console.log('Teams data:', teamsData)
			} catch (err) {
				console.error('Error fetching teams:', err)
				setError(err.response?.data?.message || 'Не удалось загрузить команду')
			} finally {
				setLoading(false)
			}
		}
	...
	const values = [
		{
			...
			title: 'Инновации',
			text: 'Постоянный поиск новых способов управления светом и цветом. Мы всегда на шаг впереди.',
			delay: 0
		},
		{
			icon: <Award size={32} strokeWidth={1.5} />,
			title: 'Качество',
			text: 'Каждый прибор проходит 48-часовое тестирование перед отправкой. Мы гарантируем надежность.',
			delay: 0.1
		},
		{
			icon: <Globe size={32} strokeWidth={1.5} />,
			title: 'Экология',
			text: 'Лидерство в разработке энергоэффективных LED-технологий. Заботимся о планете.',
			delay: 0.2
		},
		{
			icon: <Users size={32} strokeWidth={1.5} />,
			title: 'Поддержка',
			text: 'Глобальная сеть сервисных центров по всему миру 24/7. Мы всегда рядом.',
			delay: 0.3
		}
	]
				...
				<div className='max-w-[1400px] mx-auto relative z-10'>
					...
					<div className='grid grid-cols-1 lg:grid-cols-2 gap-12 items-end'>
						...
						>
							...
							пространства в незабываемые визуальные впечатления.
							Инновации — это наше ДНК, а качество — главный приоритет.
						</motion.p>
					...
					<div className='lg:col-span-5 space-y-6'>
						...
						>
							...
							<p className='text-gray-500 leading-relaxed font-medium mb-6'>
								...
								инструмент, а способ коммуникации и создания эмоций.
							</p>
