...
import {
	...
	ArrowUp,
	Code2,
} from 'lucide-react' // Добавил иконку Code2
	...
	return (
		<footer className='relative z-10 bg-[#080808] py-20 border-t border-white/5 text-white font-sans'>
			<div className='max-w-[1500px] mx-auto px-6 md:px-12'>
				...
				{/* НИЖНЯЯ ЧАСТЬ */}
				<div className='pt-10 border-t border-white/10 flex flex-col md:grid md:grid-cols-3 items-center gap-6'>
					{/* Copyright */}
					<div className='text-[11px] text-white/40 font-bold uppercase tracking-widest md:justify-self-start'>
						{t('footer_copyright')}
					</div>

					{/* ДЛЯ РАЗРАБОТЧИКОВ (Новый блок) */}
					<div className='flex items-center gap-4 text-[10px] text-white/30 font-bold uppercase tracking-widest'>
						<div className='flex items-center gap-2 hover:text-white transition-colors cursor-default'>
							<Code2 size={14} className='text-[#e21e26]' />
							<span>v1.0.4 — Prod</span>
						</div>
						<span className='opacity-20'>|</span>
						<a
							href='/api-docs'
							className='hover:text-[#e21e26] transition-colors border-b border-transparent hover:border-[#e21e26]'
						>
							API Docs
						</a>
					</div>
					...
					<button
						onClick={scrollToTop}
						className='group flex items-center gap-3 text-[11px] font-bold uppercase tracking-[0.2em] text-white transition-colors hover:text-[#e21e26] md:justify-self-end'
					>
