...
const Products = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        setLoading(true);
        // Assuming your URL pattern is 'get-product-categories/' or similar
  ...
  // Filter logic updated to use 'name' from your Django model
...
const ProductCard = ({ category, index }) => (
  <motion.div
    ...
    className="group"
  >
    {/* Updated Link to use category ID */}
    <Link to={`/subcategory/${category.id}`} className="block no-underline">
      ...
      <div className="flex flex-col">
        ...
        <p className="text-gray-400 text-[11px] font-medium leading-relaxed">
          Просмотр всех товаров в данной категории
        </p>
      </div>
    </Link>
  </motion.div>
