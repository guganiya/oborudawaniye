...
const SubCategory = () => {
  ...
  const getLocalizedName = (item, fieldBase = "name") => {
    ...
    if (lang === "en" && item[`${fieldBase}_en`])
      return item[`${fieldBase}_en`];
    if (lang === "tk" && item[`${fieldBase}_tk`])
      return item[`${fieldBase}_tk`];
  ...
  return (
    <div className="bg-white min-h-screen font-sans selection:bg-[#e21e26] selection:text-white text-[#1a1a1a]">
      ...
      <header className="relative h-[80vh] mt-30 min-h-[550px] bg-black overflow-hidden">
        ...
        <div className="absolute bottom-10 left-6 md:left-20 flex items-center gap-6 z-20">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="text-white/50 hover:text-[#e21e26] transition-colors"
            >
              <ArrowLeft size={24} />
            </button>
            <button
